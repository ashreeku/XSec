...
traindatapath <- paste("../data/", args[[2]], "/train.mat", sep="")
print (paste("Loading train data from", traindatapath))
data <- readMat(traindatapath)
# data <- readMat('./data_nsl_kdd_multi_train_0_30000.mat')
X <- data[['X']]
Y <- data[['y']]
...
if (run_train == TRUE) {
    print ("Explaining train data")
    source('lemna_explain_train_data.R')
}
print ("Explaining test data")
source('lemna_explain_test_data.R')