...
if __name__ == "__main__":
    ...
    for i, config_name in enumerate(["pdf.json", "phishing.json", "netflow.json", "bodmas.json", "nsl_kdd_multi.json"]):
        ...
        with open(f"../results/{config['dataset']}/xsec_fidelity_values_seed{seed}_k20_n3_ablationNone.pkl", "rb") as infile:
            xsec_20 = pickle.load(infile)
        with open(f"../results/{config['dataset']}/xsec_fidelity_values_seed{seed}_k30_n3_ablationNone.pkl", "rb") as infile:
            xsec_30 = pickle.load(infile)

        step = config["evaluation"]["step"]
        x_axis = np.arange(step, config["feature_dim"] + 1, step)

        synth_test_xsec_10 = xsec_10["synth_test"]
        synth_test_xsec_20 = xsec_20["synth_test"]
        synth_test_xsec_30 = xsec_30["synth_test"]
        
        axs[0][i].plot(x_axis, synth_test_xsec_10, color="blue", marker='x', linestyle="solid", label=f"k = 10")
        axs[0][i].plot(x_axis, synth_test_xsec_20, color="darkgreen", marker='o', linestyle="dashed", label="k = 20")
        axs[0][i].plot(x_axis, synth_test_xsec_30, color="red", marker='*', linestyle="dotted", label="k = 30")
        ...
        feat_aug_test_xsec_20 = xsec_20["feat_aug_test"]
        mask_xsec_20 = ~np.isnan(feat_aug_test_xsec_20)
        feat_aug_test_xsec_30 = xsec_30["feat_aug_test"]
        mask_xsec_30 = ~np.isnan(feat_aug_test_xsec_30)
        
        if np.any(mask_xsec_10):
            axs[1][i].plot(x_axis[mask_xsec_10], feat_aug_test_xsec_10[mask_xsec_10], color="blue", marker='x', linestyle="solid", label=f"k = 10")
        if np.any(mask_xsec_20):
            axs[1][i].plot(x_axis[mask_xsec_20], feat_aug_test_xsec_20[mask_xsec_20], color="darkgreen", marker='o', linestyle="dashed", label="k = 20")
        if np.any(mask_xsec_30):
            axs[1][i].plot(x_axis[mask_xsec_30], feat_aug_test_xsec_30[mask_xsec_30], color="red", marker='*', linestyle="dotted", label="k = 30")
        axs[1][i].set_xlim([0, config["feature_dim"]])
        axs[1][i].set_xticks(x_axis[::2])
        axs[1][i].grid()

        feat_deduc_test_xsec_10 = xsec_10["feat_deduc_test"]
        feat_deduc_test_xsec_20 = xsec_20["feat_deduc_test"]
        feat_deduc_test_xsec_30 = xsec_30["feat_deduc_test"]
        
        axs[2][i].plot(x_axis, feat_deduc_test_xsec_10, color="blue", marker='x', linestyle="solid", label=f"k = 10")
        axs[2][i].plot(x_axis, feat_deduc_test_xsec_20, color="darkgreen", marker='o', linestyle="dashed", label="k = 20")
        axs[2][i].plot(x_axis, feat_deduc_test_xsec_30, color="red", marker='*', linestyle="dotted", label="k = 30")
    ...
    plt.tight_layout()
    plt.savefig(f"../results/figures/fidelity_hyperparameter_sensitivity_k.pdf", bbox_inches="tight")