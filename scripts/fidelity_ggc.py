...
if __name__ == "__main__":
    ...
    logits, _, _, _ = model(x_test.unsqueeze(1).unsqueeze(3))
    pred_orig = logits.argmax(dim=1).int()

    with open(f"../results/{config['dataset']}/importance_scores/ggc_seed{seed}.pkl", "rb") as infile:
    ...
    for m in tqdm(x_axis, leave=False, desc="Synthetic Test"):
        ...
        synthetic_samples = synthetic_samples.unsqueeze(1).unsqueeze(3)
    ...
    for m in tqdm(x_axis, leave=False, desc="Feature Augmentation Test"):
        ...
        try:
            ...
            augmented_samples = augmented_samples.unsqueeze(1).unsqueeze(3)
    ...
    for m in tqdm(x_axis, leave=False, desc="Feature Deduction Test"):
        ...
        deducted_samples = deducted_samples.unsqueeze(1).unsqueeze(3)
    ...
    with open(f"../results/{config['dataset']}/ggc_fidelity_values_seed{seed}.pkl", "wb") as outfile:
