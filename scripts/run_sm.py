...
def sm_generate_importance_scores(x, y, sm_explainer):
    ...
    for i, (s, p) in enumerate(tzip(x, y)):
        ...
        exp = sm_explainer.attribute(sample, target=p.item()).detach()
        w = exp.squeeze()
...
if __name__ == "__main__":
    ...
    sm_explainer = Saliency(model.wrapped_forward)

    start_time = default_timer()
    if args["train"]:
        importance_scores = sm_generate_importance_scores(x_train, pred_train, sm_explainer)
    else:
        importance_scores = sm_generate_importance_scores(x_test, pred_test, sm_explainer)
    ...
    if args["train"]:
        with open(f"../results/{config['dataset']}/importance_scores/sm_train_seed{seed}.pkl", "wb") as outfile:
            pickle.dump(data, outfile, pickle.HIGHEST_PROTOCOL)
    else:
        with open(f"../results/{config['dataset']}/importance_scores/sm_seed{seed}.pkl", "wb") as outfile:
            pickle.dump(data, outfile, pickle.HIGHEST_PROTOCOL)