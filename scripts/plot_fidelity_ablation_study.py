...
if __name__ == "__main__":
    fig, axs = plt.subplots(nrows=3, ncols=5, sharex='col', sharey='row')
    ...
    for i, config_name in enumerate(["pdf.json", "phishing.json", "netflow.json", "bodmas.json", "nsl_kdd_multi.json"]):
        ...
        axs[0][i].plot(x_axis, synth_test_binary, color="darkgreen", marker='o', linestyle="dashed", label="w/o $\mathcal{L}_{bin}$")
        axs[0][i].plot(x_axis, synth_test_diversity, color="red", marker='*', linestyle="dotted", label="w/o $\mathcal{L}_{sim}$")
        # axs[0][i].plot(x_axis, synth_test_crossentropy, color="orange", marker='|', linestyle="dashdot", label="w/o $\mathcal{L}_{xe}$")
        axs[0][i].plot(x_axis, synth_test_cluster, color="pink", marker='^', linestyle=(0, (3, 5, 1, 5, 1, 5)), label="w/o $\mathcal{L}_{cls}$")
        axs[0][i].plot(x_axis, synth_test_sparsity, color="brown", marker='s', linestyle=(0, (3, 1, 1, 1, 1, 1)), label="w/o $\mathcal{L}_{spar}$")
        ...
        if np.any(mask_binary):
            axs[1][i].plot(x_axis[mask_binary], feat_aug_test_binary[mask_binary], color="darkgreen", marker='o', linestyle="dashed", label="w/o $\mathcal{L}_{bin}$")
        if np.any(mask_diversity):
            axs[1][i].plot(x_axis[mask_diversity], feat_aug_test_diversity[mask_diversity], color="red", marker='*', linestyle="dotted", label="w/o $\mathcal{L}_{sim}$")
        # if np.any(mask_crossentropy):
        #     axs[1][i].plot(x_axis[mask_crossentropy], feat_aug_test_crossentropy[mask_crossentropy], color="orange", marker='|', linestyle="dashdot", label="w/o $\mathcal{L}_{xe}$")
        if np.any(mask_cluster):
            axs[1][i].plot(x_axis[mask_cluster], feat_aug_test_cluster[mask_cluster], color="pink", marker='^', linestyle=(0, (3, 5, 1, 5, 1, 5)), label="w/o $\mathcal{L}_{cls}$")
        if np.any(mask_sparsity):
            axs[1][i].plot(x_axis[mask_sparsity], feat_aug_test_sparsity[mask_sparsity], color="brown", marker='s', linestyle=(0, (3, 1, 1, 1, 1, 1)), label="w/o $\mathcal{L}_{spar}$")
        ...
        axs[2][i].plot(x_axis, feat_deduc_test_binary, color="darkgreen", marker='o', linestyle="dashed", label="w/o $\mathcal{L}_{bin}$")
        axs[2][i].plot(x_axis, feat_deduc_test_diversity, color="red", marker='*', linestyle="dotted", label="w/o $\mathcal{L}_{sim}$")
        # axs[2][i].plot(x_axis, feat_deduc_test_crossentropy, color="orange", marker='|', linestyle="dashdot", label="w/o $\mathcal{L}_{xe}$")
        axs[2][i].plot(x_axis, feat_deduc_test_cluster, color="pink", marker='^', linestyle=(0, (3, 5, 1, 5, 1, 5)), label="w/o $\mathcal{L}_{cls}$")
        axs[2][i].plot(x_axis, feat_deduc_test_sparsity, color="brown", marker='s', linestyle=(0, (3, 1, 1, 1, 1, 1)), label="w/o $\mathcal{L}_{spar}$")
    ...
    # Show y-axis tick labels only on the leftmost column
    for row in range(3):
    ...
    fig.legend(handles, labels, ncol=6, loc="upper center", bbox_to_anchor=(0.5, 1.10), frameon=True, edgecolor="black", fontsize=24)
