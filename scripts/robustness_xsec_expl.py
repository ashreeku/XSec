...
from utils import set_seed, load_data, get_args, normalize_scores
...
if __name__ == "__main__":
    ...
    for orig_sample, orig_prob, orig_expl in tzip(x_test, probs_test, importance_scores):
        ...
        adv_sample = orig_sample.clone().detach().to(device)
        ...
        for it in range(num_iter):
            ...
            adv_expl = model.explain(adv_sample, n=n).unsqueeze(0)
            adv_expl = normalize_scores(adv_expl)
        ...
        with torch.no_grad():
            adv_samples.append(adv_sample.squeeze(0))
            adv_importances.append(model.explain(adv_sample, n=3).squeeze(0).cpu())
