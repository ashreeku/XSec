...
def ig_generate_importance_scores(x, y, ig_explainer):
    ...
    for i, (s, p) in enumerate(tzip(x, y)):
        ...
        exp = ig_explainer.attribute(sample, target=p.item()).detach()
        w = exp.squeeze()
...
if __name__ == "__main__":
    ...
    ig_explainer = IntegratedGradients(model.wrapped_forward)

    start_time = default_timer()
    if args["train"]:
        importance_scores = ig_generate_importance_scores(x_train, pred_train, ig_explainer)
    else:
        importance_scores = ig_generate_importance_scores(x_test, pred_test, ig_explainer)
    ...
    if args["train"]:
        with open(f"../results/{config['dataset']}/importance_scores/ig_train_seed{seed}.pkl", "wb") as outfile:
            pickle.dump(data, outfile, pickle.HIGHEST_PROTOCOL)
    else:
        with open(f"../results/{config['dataset']}/importance_scores/ig_seed{seed}.pkl", "wb") as outfile:
            pickle.dump(data, outfile, pickle.HIGHEST_PROTOCOL)