...
################ Generate test data #######
testdatapath <- paste("../data/", args[[2]], "/test.mat", sep="")
print (paste("Loading test data from", testdatapath))
datatest <- readMat(testdatapath)
# datatest <- readMat('./data_nsl_kdd_multi_test.mat')
Xtest <- datatest[['X']]
Ytest <- datatest[['y']]
...
Rcpp::sourceCpp('express.cc')
alpmcmc <- .loopexp3(helpa_final, helpb_final, helpc_final, Xtest, Ytest, J, nrow(Xtest))
...
filepath <- paste("../results/", args[[2]], "/lemna_parameters/", "test_", args[[1]], ".RData", sep="")
save(final_params, file = filepath)
